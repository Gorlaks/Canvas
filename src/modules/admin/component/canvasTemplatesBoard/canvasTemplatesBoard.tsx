...
const CanvasTemplatesBoard = (props: {
  ...
  const userAuthData: IUserAuthData = localStorageApi.getLocalData(
    "userAuthData",
    {}
  );
  return (
    <div className="canvas-templates-board">
      {Boolean(data.length) &&
        data.map((item) => {
          const canvasId = item["_id"];
          const canvasType = item["type"];
          return (
            <div
              key={canvasId}
              className="canvas-templates-board__template-wrapper"
            >
...
const handleDeleteTemplate = (
  ...
  adminService
    ...
    .then((response: IServerResponse) => {
      ...
      const filteredList = data.filter((item: any) => item["_id"] !== canvasId);
